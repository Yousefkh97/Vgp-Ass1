...
Eigen::Vector4f getP(Eigen::Vector3f bc, int fid, Eigen::Matrix4f view, Eigen::MatrixXi F, Eigen::MatrixXd V)
{
	Eigen::Vector4f p1 = Eigen::Vector4f(V(F(fid, 0), 0), V(F(fid, 0), 1),V(F(fid, 0), 2), 1);
	Eigen::Vector4f p2 = Eigen::Vector4f(V(F(fid, 1), 0), V(F(fid, 1), 1),V(F(fid, 1), 2), 1);
	Eigen::Vector4f p3 = Eigen::Vector4f(V(F(fid, 2), 0), V(F(fid, 2), 1),V(F(fid, 2), 2), 1);
	p1 = view * p1;
	p2 = view * p2;
	p3 = view * p3;
	return p1 * bc(0) + p2 * bc(1) + p3 * bc(2);
}
...
float Renderer::Picking(double newx, double newy)
{
		...
		{
			ans = getP(bc, fid, view, scn->data().F, scn->data().V);
			return ans(2);
		}
		return -11110;
}
